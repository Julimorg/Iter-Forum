...
import authorizedAxiosInstance from '../../services/Auth'; // Sử dụng axios instance có sẵn từ PostDetail
...
const ReportPanel: React.FC<ReportPanelProps> = ({ onClose, type, user_id, post_id, comment_id }) => {
  ...
  const handleConfirmReport = async () => {
    ...
    const reportData: ReportRequest = {
      ...
      report_body: description || '', // Nếu không có description thì gửi chuỗi rỗng
    ...
    <PanelOverlay>
      <ReportPanelContainer className="report-panel-container">
        {reportSubmitted ? (
          ...
          <>
            <Title>{selectedReason ? `More detail - ${selectedReason}` : 'Choose your reasons'}</Title>
            {selectedReason ? (
              <>
